...
The first check prevents very small channels from being used in the framework, as they are not large enough to carry the \fullref{sec:microprotocols}. The second check prevents too many channels from being used, as the micro protocols need to be able to uniquely identify each one, and if there are too many channels then they cannot be used (but since they have been registered they can still be considered when picking an algorithm, causing a failure in the communication).
